# copy and paste the encrypted message here
message = 'kurzytqoncpsipjyvsftcmazclpn'
# copy and paste the secret here
sec = ['w', 't', 'g', 'b', 't', 'd', 'g', 'c', 'o', 'n', 'n', 'g', 't', 't', 'n', 'v', 'r', 'g', 'm', 'i', 'i', 'l', 'd', 'q', 'j', 'k', 'b', 'o']

# Array for Decryption
...
def letter_from_position(position):
    if position < 1 or position > 26:
        return "Position must be between 1 and 26"
    return chr(position + 96)
...
while loop < length:
    if secret[loop][search] == message[loop]:
        plaintext = plaintext + (letter_from_position(search))
        loop += 1
        search = 1
    else:
        search += 1
